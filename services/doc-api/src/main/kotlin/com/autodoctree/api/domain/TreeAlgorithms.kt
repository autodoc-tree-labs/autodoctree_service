...
class NeighborBuilder(
    ...
    fun build(workspaceId: String, documents: List<DocumentRow>, embeddings: Map<String, EmbeddingRow>, topK: Int): NeighborGraph {
        ...
        documents.forEach { document ->
            ...
            val neighbors = documents
                ...
                .mapNotNull { candidate ->
                    ...
                    NeighborLink(candidate.id, cosine(source, candidateVector))
                }
...
class TreeLabeler {
    ...
    fun labelClusters(workspaceDocuments: List<DocumentRow>, clusters: List<TreeCluster>): Map<String, String> {
        ...
        return clusters.associate { cluster ->
            ...
            val clusterDocs = workspaceDocuments.filter { cluster.documentIds.contains(it.id) }
            clusterDocs.forEach { document ->
                tokenize(document.title + " " + (document.bodyText ?: "")).forEach { token ->
            ...
            val bestTerms = tf.entries
                .map { (token, freq) ->
                    ...
                    token to (freq.toDouble() * idf)
                }
                .sortedByDescending { it.second }
                .map { it.first }
                .take(3)

            val label = sanitizeLabel(bestTerms)
            cluster.id to label
        }
    }
