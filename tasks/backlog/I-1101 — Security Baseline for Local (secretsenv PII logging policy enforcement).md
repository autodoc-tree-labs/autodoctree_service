# I-1101 — Security Baseline for Local (secretsenv PII logging policy enforcement)
...
## Goal
로컬에서도 민감정보 로그 금지/시크릿 관리 기준을 강제한다.

## Context
디버그/설명 기능이 늘면 실수로 본문이 로그에 나갈 수 있다. 정책을 코드 레벨에서 막아야 한다.

## Scope
- log sanitizer(본문 길이 제한, 특정 필드 차단)
- .env 관리 가이드
- pre-commit(옵션)로 secrets 스캔

## Non-goals
기업 보안 인증

## Deliverables
Sanitizer + docs + optional hooks

## Acceptance Criteria
본문/첨부 텍스트가 로그에 찍히지 않음(테스트로 보장)
...
## Test Plan
unit: sanitizer / integration: 금지 필드 포함 로그 방지
